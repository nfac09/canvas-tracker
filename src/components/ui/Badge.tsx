...
export function Badge({ children, className = '' }: BadgeProps) {
  return (
    <span
      className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md text-[11px] font-medium ${className}`}
    >
      {children}
    </span>
  )
...
export function StatusBadge({ status }: { status: AssignmentStatus }) {
  const classes = {
    not_started: 'bg-slate-100 text-slate-500 dark:bg-white/[0.07] dark:text-slate-400',
...
export function CategoryBadge({ category }: { category: AssignmentCategory }) {
  const classes: Record<AssignmentCategory, string> = {
    discussion: 'bg-violet-50 text-violet-700 dark:bg-violet-500/10 dark:text-violet-400',
    quiz: 'bg-orange-50 text-orange-700 dark:bg-orange-500/10 dark:text-orange-400',
    lab: 'bg-cyan-50 text-cyan-700 dark:bg-cyan-500/10 dark:text-cyan-400',
    exam: 'bg-red-50 text-red-700 dark:bg-red-500/10 dark:text-red-400',
    homework: 'bg-indigo-50 text-indigo-700 dark:bg-indigo-500/10 dark:text-indigo-400',
    other: 'bg-slate-100 text-slate-600 dark:bg-white/[0.07] dark:text-slate-400',
  }
  return (
...
export function CourseBadge({ name, color }: { name: string; color: string }) {
  return (
    <span
      className="inline-flex items-center px-1.5 py-0.5 rounded-md text-[11px] font-medium text-white"
...
export function GradeBadge({ pointsEarned, pointsPossible, letterGrade }: {
  ...
  if (letterGrade && !pointsPossible) {
    return <Badge className="bg-indigo-50 text-indigo-700 dark:bg-indigo-500/10 dark:text-indigo-400">{letterGrade}</Badge>
  }
  if (pointsEarned !== undefined && pointsPossible) {
    ...
    const color =
      ...
      pct >= 80 ? 'bg-blue-50 text-blue-700 dark:bg-blue-500/10 dark:text-blue-400' :
      pct >= 70 ? 'bg-amber-50 text-amber-700 dark:bg-amber-500/10 dark:text-amber-400' :
